...
// *** Form Lleida *** //
		...
		$("#form-lleida").on("submit", function (event) {
			if (event.isDefaultPrevented()) {
				var errorContent = '<i class="cs-error-icon fas fa-times"></i>' +
					errorMsg;
				lleidaSubmitMSG(false, errorContent);
				lleidaError();
				console.log("Error");
	...
	function lleidaSubmitForm() {
		...
		// 		if (!res.error) {
		// 			console.log("éxito Lead Captado");
		// 			lleidaSuccess();
		// 		} else {
		// 			console.log("Error PHP");
		// 			lleidaError();
		// 			lleidaSubmitMSG(false, text);
		// 		}
		// 	}
		// });
	}
...
function cs2SubmitForm() {
	...
	$.ajax({
		type: "POST",
		url: "https://backendtabacos.sevisl.com/api/users/registerUser",
		data: "name=" + name + "&email=" + email + "&phone=" + phoneNum,
		success: function (response) {
			var res = JSON.parse(response);
			console.log(res.error);
			if (!res.error) {
